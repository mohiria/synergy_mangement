(function () {
  ...
  const entryApprovalText = (data, item) => `待${member(data, kr(data, item?.krId)?.owner).displayName}审批`;
  const taskStatusText = (data, item) => item?.status === "待入池审批" ? entryApprovalText(data, item) : item?.status || "—";
  const riskReasonText = (data, item) => item?.reason === "任务等待入池审批" ? `任务${entryApprovalText(data, task(data, item.taskId))}` : item?.reason || "";
  ...
  const nameOf = (data, id) => task(data, id)?.title || kr(data, id)?.title || objective(data, id)?.title || member(data, id)?.displayName || riskReasonText(data, risk(data, id)) || id;
  ...
  function krRiskReason(data, itemKr) {
    ...
    return riskReasonText(data, data.risks.find((item) => ids.has(item.taskId)));
  }
  ...
  function nodeMarkup(node) {
    ...
    if (node.type === "objective") return `<g class="${cls}" data-cp-node="${node.id}" data-action="cp-node" data-id="${node.id}" transform="translate(${node.x} ${node.y})"><rect x="-125" y="-36" width="250" height="72" rx="4"/><text class="node-code" text-anchor="middle" y="-8">${node.id}</text><text class="node-caption" text-anchor="middle" y="17">${esc(node.label)}</text><title>${esc(`${node.id} · ${node.label}`)}</title></g>`;
    if (node.type === "kr") {
      const riskText = node.blockerCount ? `${riskLabel(node.risk)} · ${node.blockerCount} 卡点` : riskLabel(node.risk);
      const subtitle = node.subtitle || "", shortSubtitle = subtitle.length > 10 ? `${subtitle.slice(0, 10)}…` : subtitle;
      const riskMarker = ["warning", "risk"].includes(node.risk) ? `<g class="node-risk-marker ${node.risk}" transform="translate(76 -34)"><circle r="9"/><text text-anchor="middle" y="4">!</text></g>` : "";
      return `<g class="${cls}" data-cp-node="${node.id}" data-action="cp-node" data-id="${node.id}" transform="translate(${node.x} ${node.y})"><rect x="-88" y="-42" width="176" height="84" rx="4"/><text class="node-code" x="-68" y="-14">${node.id}</text><text class="node-caption" x="-68" y="9">${esc(shortSubtitle)}</text><text class="node-risk-line ${node.risk || "normal"}" x="-68" y="31">${esc(riskText)}</text>${riskMarker}<title>${esc(`${node.id} · ${subtitle} · ${riskText}`)}</title></g>`;
    }
  ...
  function edgePath(edge, from, to, model) {
    if (edge.kind === "owns" && from.type === "objective" && to.type === "kr") {
      ...
      return `M ${from.x} ${from.y + 36} V ${branchY} H ${to.x} V ${to.y - 42}`;
    }
  ...
  function renderNodeDetail(data, node) {
    ...
    if (itemTask) {
      ...
      return `<div class="cp-inspector-title"><span class="cp-eyebrow">任务节点 · ${itemTask.id}</span><h2>${esc(itemTask.title)}</h2>${statusBadge(taskStatusText(data, itemTask), taskStatusKey(itemTask.status))}${taskRiskKey !== "normal" ? statusBadge(riskLabel(taskRiskKey), taskRiskKey) : ""}</div>
        ...
        ${itemRiskFact ? `<div class="cp-risk-fact ${taskRiskKey}"><span>结构化卡点 · ${esc(itemRiskFact.level)}</span><b>${esc(riskReasonText(data, itemRiskFact))}</b><small>待行动人：${esc(member(data, itemRiskFact.actionOwner).displayName)} · 已持续 ${itemRiskFact.days} 天</small><small>${esc(itemRiskFact.impact)}</small></div>` : ""}
    ...
    if (itemKr) {
      ...
      return `<div class="cp-inspector-title"><span class="cp-eyebrow">KR 节点 · ${itemKr.id}</span><h2>${esc(itemKr.title)}</h2>${statusBadge(riskLabel(itemKr.risk), itemKr.risk)}</div><div class="cp-property-grid">${property("负责人", esc(member(data, itemKr.owner).displayName))}${property("任务数量", `${tasks.length} 项`)}${property("卡点数量", `${itemRisks.length} 项`)}${property("周期", esc(itemKr.cycle))}</div>${itemRisks.length ? `<div class="cp-risk-fact ${itemKr.risk}"><span>${riskLabel(itemKr.risk)}原因</span><b>${esc(riskReasonText(data, itemRisks[0]))}</b><small>${esc(itemRisks[0].impact)}</small></div>` : ""}<div class="cp-detail-block"><div class="cp-block-head"><b>量化指标</b></div><p>${esc(itemKr.metric)}</p></div>`;
    }
    if (itemO) {
    ...
    if (itemRisk) {
      ...
      return `<div class="cp-inspector-title"><span class="cp-eyebrow">结构化卡点 · ${itemRisk.id}</span><h2>${esc(riskReasonText(data, itemRisk))}</h2>${statusBadge(itemRisk.level, itemRisk.level === "高风险" ? "risk" : "warning")}</div><div class="cp-property-grid">${property("所属任务", `${itemRisk.taskId} · ${esc(belongs?.title || "")}`)}${property("待行动人", `${esc(owner.displayName)} · ${esc(owner.role)}`)}${property("已持续", `${itemRisk.days} 天`)}${property("影响范围", esc(itemRisk.impact))}</div><div class="cp-impact"><span>系统推导 · 硬前置影响路径</span><b>${affected.os.join("、") || "暂无上层目标影响"}</b><b>${affected.krs.join("、") || "暂无下游 KR 影响"}</b></div>`;
    }
    if (itemMember) {
  ...
  function searchCandidates(data) {
    ...
    const nodes = [
      ...
      ...data.risks.map((item) => ({ kind: "node", id: item.id, type: "卡点", title: riskReasonText(data, item) })),
    ];
