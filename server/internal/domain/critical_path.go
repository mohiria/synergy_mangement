...
// RequiredEdgeAnalysis 分析结果：互锁边集、关键路径边集与是否可计算。
type RequiredEdgeAnalysis struct {
	Interlocked           map[int64]bool
	OnCriticalPath        map[int64]bool
	CriticalPathAvailable bool
}

// AnalyzeRequiredEdges 分析硬前置边（AC-10）：
// 1) 循环检测——反复剥离入度或出度为 0 的节点，剩余节点即在循环中；两端都在循环中的边标互锁；
// 2) 互锁边被排除（循环部分暂停传统关键路径计算）；
// 3) 剩余 DAG 上以任务工期为权做最长路径；任一相关任务缺工期则只确认硬依赖链、不宣称关键路径。
func AnalyzeRequiredEdges(edges []RequiredEdge, durationDays map[int64]int) RequiredEdgeAnalysis {
	res := RequiredEdgeAnalysis{Interlocked: map[int64]bool{}, OnCriticalPath: map[int64]bool{}}
	if len(edges) == 0 {
	...
	dag := make([]RequiredEdge, 0, len(edges))
	...
	queue := []int64{}
	dist := map[int64]int{}
	prevEdge := map[int64]*RequiredEdge{}
	for n := range involved {
		if indeg[n] == 0 {
