...
func TestSubmitCompletionRule(t *testing.T) {
	...
	cases := []struct {
		...
		{"已在终审不可重复提交", facts(TaskInReview), 1, "x", ErrCompletionNotInProgress},
	}
...
// 任一人通过→已完成，退回→进行中且退回意见必填；普通成员与任务负责人不可终审。
func TestDecideCompletionRule(t *testing.T) {
	pending := TaskFacts{Status: TaskInReview, CreatorID: 3, OwnerID: 5}
