...
func TestAnalyzeRequiredEdges(t *testing.T) {
	...
	t.Run("无循环时计算关键路径", func(t *testing.T) {
		...
		res := AnalyzeRequiredEdges(edges, dur)
	...
	t.Run("日期不足时只显示硬依赖链", func(t *testing.T) {
		edges := []RequiredEdge{{ID: 31, Source: 1, Target: 2}}
		partial := map[int64]int{1: 5} // 任务 2 无工期
		res := AnalyzeRequiredEdges(edges, partial)
		if res.CriticalPathAvailable {
