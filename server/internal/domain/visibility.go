...
// handler 不各写一遍。顺序是显式身份优先——项目负责人 → 成员角色 → 公开项目的隐式访客 →
// 无身份。隐式访客只在项目公开且此人没有任何显式身份时产生，读权限与显式访客相同，
// 写动作一律不放行（含发表讨论），也不能被指定为接收方，差别都靠 Implicit 这一位表达。
func ProjectIdentity(userID, ownerID int64, memberRole, visibility string) Actor {
	if userID == ownerID {
