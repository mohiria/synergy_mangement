...
func TestRenderReportHTML(t *testing.T) {
	...
	for _, want := range []string{
		...
		"一、本期成果", "<th>交付物</th><th>任务</th>", "<div>验收方案V1.docx</div>", "无交付物文件", "进度 40%",
		"二、风险与卡点", "另有 2 件完成审核仍在审批队列", "<th>任务</th><th>卡点</th>",
		`<td class="kind"><b>上游未就绪</b><span class="tag new">本期新出现</span></td>`, "<b>7</b> 天",
		"本期解除 1 项", "任务超期，2026-09-10 解除）",
		"三、下一步", "到期／超期", `<div class="red">超期 3 天</div>`, "即将启动", "附、O／KR 进展", "高风险", "未填",
	} {
	...
	for _, gone := range []string{"生效 2026", "必要输入未就绪", "上游未完成任务", "上游未就绪：", "KR1.1 · 上线自动验收", "已填", "交付物：", "<b>临近截止任务</b>"} {
