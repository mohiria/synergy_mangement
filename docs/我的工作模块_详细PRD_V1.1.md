...
| 等待他人 | ① 我任务的上游：上游任务（含替他人创建的上游任务，#178）；② 我已提交、停在他人审批队列中的事项：关闭申请、完成申请（显示当前环节，#172 修订） |
...
| 事项 | 进入条件 | 退出条件 | 卡片动作 |
|---|---|---|---|
| 上游任务 | 本人负责任务存在必要输入未就绪，来源为上游任务（未完成或已取消；含替他人创建的上游任务，#178）；与本任务是否已开始无关 | 上游任务完成终审（输入就绪）；本任务改换来源生效 | 提醒上游任务负责人 |
...
```mermaid
flowchart TD
    X["一条事项（任务 / 审批单 / 待接收项 / 卡点）"] --> Q1{"我是本事项的审批人？<br/>关闭申请 / 中间审核 / KR 终审"}
    Q1 -- "是" --> G2["待我审批"]
...
6. 任务取消：未决审批单自动关闭，相关卡片全部消失；下游任务对应输入标为“上游未就绪：来源已取消”，该原因在下游任务详情中展示。
...
- 取消权限：任务负责人、所属 KR 负责人、项目管理员，必填理由；取消时未决审批单自动关闭；
...
站内通知触发点固定：任务入池（通知所属 KR 负责人，裁决 #162）、任务字段修改（通知所属 KR 负责人，#172 修订）、关闭申请待办／结果、完成审批待办／退回／终审通过、被指定为上游任务负责人（#178）、上游任务已完成（通知下游）、一键提醒。
...
| MW-12 | 卡点 | 任务超期卡点出现在负责人与所属 KR 负责人的卡点组；审批超时卡点出现在审批人卡点组 |
