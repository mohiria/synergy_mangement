...
2. **输入请求通知时点前移**：输入关系建立后立即通知对接人并进入其“待我处理”，不再等待入池通过（#172 起输入源修改直接生效，无“待审批阶段”）。
...
| KR 负责人 | 待我审批（关闭申请、KR 终审，#172 修订）、与我相关的卡点（本人 KR 下有卡点、互锁待行动人） |
| 中间审核人 | 待我审批（中间审核或签） |
...
| 待我审批 | 本人需处理的关闭申请、中间审核（或签）和 KR 终审（#172 修订：关键字段修改直接生效，不再进审批） |
| 待我接收 | 本人作为接收方需要确认接收的已交付文件 |
| 等待他人 | ① 我任务的上游：上游任务、我发起的输入请求；② 我已提交、停在他人审批队列中的事项：关闭申请、完成申请（显示当前环节，#172 修订） |
...
| 关闭申请被退回的任务 | 关闭申请状态 = 已退回且提交人 = 本人；形成“退回待处理事项”，退回理由保留在任务详情（#172 修订） | 重新发起、明确放弃本次关闭申请，或任务终止；仅查看不退出 |
...
| 关闭申请 | 本人发起的关闭申请状态 = 待审批（任务仍正常推进，仍在待我处理，#172 修订） | 通过／退回 | 提醒 KR 负责人 |
...
```mermaid
flowchart TD
    X["一条事项（任务 / 审批单 / 输入请求 / 待接收项 / 卡点）"] --> Q1{"我是本事项的审批人？<br/>关闭申请 / 中间审核 / KR 终审"}
    Q1 -- "是" --> G2["待我审批"]
    ...
    Q3 -- "是" --> G1["待我处理"]
    Q3 -- "否" --> Q4{"我是任务负责人 / 创建人？"}
    Q4 -- "是" --> Q5{"任务当前停在他人审批队列？<br/>关闭申请待审批 / 完成审批中"}
...
1. 关闭申请待审批**不改变**任务归组（#172 修订）：任务仍在待我处理（照常执行），申请本身作为一条独立事项进入等待他人。
2. 已提交完成申请、审批中的本人任务只在等待他人，不在待我处理。
3. 被退回事项（关闭申请退回、完成申请退回）回到提交人的待我处理，退回理由在任务详情中定位展示。
...
9. 关闭申请被退回后形成退回待处理事项（#172 修订）；仅查看不清除，重新发起、明确放弃或任务终止后退出。
...
| 关闭申请、中间审核、KR 终审 | 审核，定位当前审批单 | “去处理”文字按钮 |
...
- **审核**：关闭申请、完成申请、候选交付物和或签链，只对当前待行动审核人提供通过／退回（#172 修订）；
...
| 关闭申请 | 任务关闭的审批事实（#172 修订，旧「关键字段变更单」概念取消） | 任务、关闭原因、提交人、KR 负责人、状态、意见、提交时间、处理时间 |
...
- 关键字段修改直接生效（#172 修订），不改变任务页面状态；关闭申请审批不改变任务页面状态；
...
#### B. 关键字段修改（审批已取消，#172 修订）＋关闭申请

关键字段：标题、描述、量化指标、任务负责人、开始／截止时间、输入来源、接收方。有编辑权限者（任务负责人／所属 KR 负责人／可编辑项目者）直接修改立即生效，无修改原因；动作进任务动态并站内通知所属 KR 负责人（本人修改不另发）。中间审核人调整不是关键字段，直接生效并留痕的口径不变。

任务关闭的审批独立保留为关闭申请（原因必填）：

| 状态 | 含义 |
|---|---|
| 无申请 | 当前没有待处理关闭申请 |
| 待审批 | 等待所属 KR 负责人处理，任务正常推进 |
| 已通过 | 任务进入已关闭并保留原因 |
| 已退回 | 任务继续执行 |

KR 负责人在本人负责 KR 下关闭免审即时生效；关闭申请审批期间不能修改任务字段。
...
- 上游任务被取消时，下游对应输入标为“上游未就绪：来源已取消”并通知下游负责人，由其直接更换来源（#172 修订）；
- 更换上游任务或对接人属于关键字段修改，直接生效并留痕。
...
站内通知触发点固定：任务入池（通知所属 KR 负责人，裁决 #162）、任务字段修改（通知所属 KR 负责人，#172 修订）、关闭申请待办／结果、完成审批待办／退回／终审通过、输入请求、对接人同意接收、上游任务已完成（通知下游）、一键提醒。
...
| 权限级别 | 能力边界 |
|---|---|
| 项目管理员 | 管理结构、成员、配置；修改任意任务关键字段（#172 修订：直接生效）；不能替代中间审核人或 KR 负责人 |
| 成员 | 查看完整项目，创建任务，处理本人承担的职责动作 |
| 访客 | 只看，不改变项目事实 |

| 职责 | 决定的动作 |
|---|---|
| KR 负责人 | 关闭申请审批、完成终审（本人 KR）；接收本人 KR 下任务入池与字段修改通知（裁决 #162／#172） |
| 任务负责人 | 开始、更新进度、修改关键字段、配置输入来源／接收方／中间审核人、上传交付物文件、提交完成申请、发起关闭申请 |
| 中间审核人 | 或签通过／退回 |
| 接收方 | 确认接收 |
| 输入对接人 | 同意接收、提交内容 |
...
| 任务字段修改（#172 修订） | 无对应事项；仅站内通知所属 KR 负责人 |
| 关闭申请待办／结果 | 待我审批／待我处理 |
...
| MW-06 | 关闭申请（#172 修订，旧「关键字段变更」） | 发起后任务仍在待我处理；申请出现在提交人的等待他人和 KR 负责人的待我审批；退回后形成退回待处理事项，理由在详情展示，仅在重新发起、明确放弃或任务终止后退出，单纯查看不清除 |
...
| MW-11 | 输入请求（已作废，#172 修订） | 输入源修改直接生效，不再有「变更待审批」阶段；请求随关系建立立即出现（见 MW-10） |
