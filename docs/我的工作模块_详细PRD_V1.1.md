...
| KR 负责人 | 待我审批（KR 终审，裁决 10 后无关闭申请）、与我相关的卡点（本人 KR 下有卡点、互锁待行动人） |
| 中间审核人 | 待我审批（中间审核或签） |
...
| 待我审批 | 本人需处理的中间审核（或签）和 KR 终审（裁决 10 后无关闭申请；#172 后关键字段修改直接生效，不进审批） |
...
```mermaid
flowchart TD
    X["一条事项（任务 / 审批单 / 待接收项 / 卡点）"] --> Q1{"我是本事项的审批人？<br/>中间审核 / KR 终审"}
    Q1 -- "是" --> G2["待我审批"]
...
| 中间审核、KR 终审 | 审核，定位当前审批单 | “去处理”文字按钮 |
...
关键字段：标题、描述、量化指标、任务负责人、开始／截止时间、输入来源、接收方。项目管理员（含项目负责人）直接修改立即生效，无修改原因；动作进任务动态并站内通知所属 KR 负责人（本人修改不另发）。中间审核人调整不是关键字段（负责人保留配置权），直接生效并留痕的口径不变。
...
站内通知触发点固定：任务入池（通知所属 KR 负责人，裁决 #162）、任务字段修改（通知所属 KR 负责人，#172 修订）、完成审批待办／退回／终审通过、被指定为上游任务负责人（#178）、上游任务已完成（通知下游）、一键提醒（裁决 10 后无关闭申请类通知）。
...
| 职责 | 决定的动作 |
|---|---|
| KR 负责人 | 完成终审（本人 KR）；接收本人 KR 下任务入池与字段修改通知（裁决 #162／#172；裁决 10 后无关闭申请审批） |
