(function () {
  ...
  function renderOverview() {
    const objectives = data.objectives.map((objective) => {
      ...
      return `<section class="card objective">
        <div class="objective-head"><span class="objective-code">${objective.id}</span><div><h2>${esc(objective.title)}</h2><p>${esc(objective.note)}</p></div><span class="objective-count">${krs.length} 个 KR</span></div>
    ...
    return `${head("项目总览", projectMeta, actions)}<div class="overview-list">${objectives}</div>`;
  }
  function renderOverviewKr(kr) {
    ...
    const riskNote = kr.risk === "normal" ? "" : risk ? `${risk.reason} · ${risk.impact}` : "存在待处理的输入或审批卡点";
    ...
    return `<div class="kr-row"><button class="kr-main" data-action="toggle-kr" data-id="${kr.id}"><span class="risk-stripe ${kr.risk}"></span><span class="kr-code">${kr.id}</span><span class="kr-title-cell"><span>${esc(kr.title)}</span>${riskNote ? `<small>${riskLabel}：${esc(riskNote)}</small>` : ""}</span>${status(kr.risk === "risk" ? "高风险" : kr.risk === "warning" ? "预警" : "正常", kr.risk)}<span>${icon(expanded ? "down" : "chevron")}</span></button>
      ${expanded ? `<div class="kr-tasks"><div class="kr-meta">负责人 ${memberName(kr.owner)} · 周期 ${kr.cycle} · 量化指标：${esc(kr.metric)}</div>${tasks.map((task) => `<div class="mini-task"><span class="mono">${task.id}</span><span>${esc(task.title)}</span><span>${ownerCell(task.owner)}</span><span>${status(task.status)}</span><span class="optional">${task.progress == null ? "未填进度" : `${task.progress}%`}</span><button class="icon-btn" data-action="task-detail" data-id="${task.id}" aria-label="查看任务">${icon("chevron")}</button></div>`).join("")}<div style="margin-top:8px"><button class="link-btn" data-action="focus-graph" data-id="${kr.id}">在协作全景中查看 ${kr.id} 影响链 →</button></div></div>` : ""}</div>`;
  }
  ...
  function renderTasks() {
    ...
    const rows = groups.map(({ kr, tasks }) => `<tr class="table-group"><td colspan="9">${kr.id}　${esc(kr.title)}　<span class="meta">${tasks.length} 项</span></td></tr>${tasks.map((task) => `<tr><td><span class="tree-indent"><span class="mono">${task.id}</span></span></td><td><button class="link-btn" data-action="task-detail" data-id="${task.id}">${esc(task.title)}</button></td><td>${ownerCell(task.owner)}</td><td>${status(task.status)}</td><td>${task.progress == null ? "—" : `<div class="progress"><i style="width:${task.progress}%"></i></div><span class="meta">${task.progress}%</span>`}</td><td>${task.start}</td><td>${task.due}</td><td>${esc(task.outputName)}</td><td><button class="icon-btn" data-action="task-detail" data-id="${task.id}" aria-label="打开详情">${icon("chevron")}</button></td></tr>`).join("")}`).join("");
    return `${head("全部任务", "按 O / KR 组织三级任务；任务创建后先提交入池审批，通过后进入执行池。", `${canCreate() ? `<button class="btn" data-action="invite-owner">${icon("user")}邀请负责人完善</button><button class="btn primary" data-action="create-task">${icon("plus")}创建任务</button>` : ""}`)}
      <div class="toolbar"><div class="toolbar-group"><label class="search-box">${icon("search")}<input id="task-search" value="${esc(ui.taskSearch)}" placeholder="搜索任务、编号或负责人" /></label><select id="task-kr" class="field"><option value="all">全部 KR</option>${data.krs.map((kr) => `<option value="${kr.id}" ${ui.taskKr === kr.id ? "selected" : ""}>${kr.id}</option>`).join("")}</select><select id="task-status" class="field"><option value="all">全部状态</option>${["未开始","进行中","等待输入","待入池审批","待中间审核","待 KR 终审","已完成"].map((x) => `<option ${ui.taskStatus === x ? "selected" : ""}>${x}</option>`).join("")}</select></div><div class="toolbar-actions"><button class="btn compact" data-action="more-feedback">批量指派</button><button class="btn compact" data-action="more-feedback">批量改期</button></div></div>
      <div class="data-table-wrap"><table class="data-table"><thead><tr><th>编号</th><th>任务</th><th>负责人</th><th>状态</th><th>可选进度</th><th>开始</th><th>截止</th><th>预期交付物</th><th></th></tr></thead><tbody>${rows || `<tr><td colspan="9"><div class="empty">没有匹配任务</div></td></tr>`}</tbody></table></div>`;
  }
  ...
  function graphModel() {
    ...
    const riskNodes = ui.graphMode === "full" ? data.risks.filter((risk)=>taskIds.includes(risk.taskId)).map((risk,index)=>({id:risk.id,label:risk.id,subtitle:risk.reason,type:"risk",x:1180,y:120+index*105,risk:"risk"})) : [];
  ...
  function renderGraphInspector(node, model) {
    ...
    return `<aside class="graph-inspector"><div class="inspector-head"><div><span class="pill">${kind}</span><h3 style="margin:8px 0 0">${esc(title)}</h3></div><button class="icon-btn" data-action="clear-graph-selection" aria-label="清除选择">${icon("close")}</button></div><div class="inspector-body">
      ...
      ${task ? `<div class="property"><label>负责人 / 状态</label><strong>${name(task.owner)}　${status(task.status)}</strong></div><div class="property"><label>预期交付物</label><strong>${esc(task.outputName)}</strong></div><div class="property"><label>周期</label><strong>${task.start} — ${task.due}</strong></div>` : ""}
  ...
  function workApprovalItems(id) {
    return [
      ...data.entryApprovals.filter((x)=>x.krOwner===id && x.state==="待审批").map((x)=>({key:`entry:${x.id}`,type:"approval",kind:"入",taskId:x.taskId,recordId:x.id,title:`入池审批 · ${x.taskId} ${getTask(x.taskId)?.title || ""}`,why:"你是所属 KR 负责人，需要判断任务是否进入执行池",need:x.reason,impact:"通过后进入执行池；退回则返回创建人",due:getTask(x.taskId)?.due,waitingDays:x.waitingDays,tab:"audit",focusType:"approval",focusId:x.id})),
  ...
  function workItems(tab) {
    ...
    if (tab === "waiting") {
      ...
      const entry=data.entryApprovals.filter((x)=>x.submitter===id && x.state==="待审批").map((x)=>completeWorkItem({key:`entry-wait:${x.id}`,type:"waiting",kind:"等",taskId:x.taskId,title:`等待入池审批 · ${x.taskId} ${getTask(x.taskId)?.title || ""}`,why:"你已提交任务，当前停在所属 KR 负责人",need:"审批通过前任务不进入执行池",impact:"通过后任务进入未开始",due:getTask(x.taskId)?.due,waitingDays:x.waitingDays,tab:"overview",focusType:"waiting-approval",focusId:x.id,reminderTarget:x.krOwner}));
  ...
  function renderWorkCard(item,tab) {
    ...
    const taskState=task?.status || invite?.state || "待处理";
    const reminder=(["waiting","risks"].includes(tab) && item.reminderTarget)?`<button class="btn compact" data-action="work-remind" data-task-id="${item.taskId}" data-target="${item.reminderTarget}" ${reminderDisabled(item)?"disabled":""}>${icon("bell")}${reminderDisabled(item)?(item.reminderTarget===ui.identity?"无需提醒本人":"今日已提醒"):"提醒"}</button>`:"";
    return `<article class="work-item ${item.overdue?"overdue":""} ${item.blocked?"blocked":""}" data-action="${openAction}" data-id="${item.taskId}" data-tab="${item.tab || "overview"}" data-source="${tab}" data-focus-type="${item.focusType || "task"}" data-focus-id="${item.focusId || ""}" data-invite-id="${item.inviteId || ""}" tabindex="0" role="button" aria-label="${esc(actionLabel)}：${esc(cardTitle)}"><div class="work-kind">${item.kind}</div><div class="work-main"><div class="work-title-row"><h3>${esc(cardTitle)}</h3></div><div class="work-meta"><span>${krId}</span><span>日期 ${cardDate}</span>${status(taskState)}</div></div><div class="actions">${reminder}<button class="btn compact ${tab==="todo"||tab==="approvals"?"primary":""}" data-action="${openAction}" data-id="${item.taskId}" data-tab="${item.tab || "overview"}" data-source="${tab}" data-focus-type="${item.focusType || "task"}" data-focus-id="${item.focusId || ""}" data-invite-id="${item.inviteId || ""}">${actionLabel}${icon("chevron")}</button></div></article>`;
  }
  function renderMyWork() {
    ...
    return `${head("我的工作","跨项目汇总当前轮到我、或我正在等待他人的事项；卡片不直接改变业务状态。","")}<section class="work-identity"><div>${avatar(member.id)}<div><b>${member.displayName}</b><span>${permission} · ${member.team}</span></div></div><p><span>当前职责</span>${esc(workResponsibilities(member.id))}</p></section><div class="work-tabs">${tabs.map(([id,label])=>`<button class="${ui.myTab===id?"active":""}" data-action="my-tab" data-value="${id}">${label}<span class="pill">${workItems(id).length}</span></button>`).join("")}</div><div class="work-list">${items.length?items.map((item)=>renderWorkCard(item,ui.myTab)).join(""):`<div class="card empty"><b>当前没有${tabs.find(([id])=>id===ui.myTab)[1]}事项</b><span>事项会随任务、审批、接收、输入和卡点事实自动进入或退出。</span></div>`}</div>`;
  }
  ...
  function renderArtifacts() {
    ...
    return `${head("成果与归档", "按 O / KR 归集当前交付物、审核中的候选内容和来源关系；不保留历史文件。", `${canCreate()?`<button class="btn" data-action="upload-attachment">${icon("upload")}上传过程文件</button>`:""}`)}
      ...
      <section style="margin-top:18px"><div class="page-head" style="margin-bottom:10px"><div><h1 style="font-size:16px">已形成的阶段成果包</h1><p>保留成果目录与来源事实；下载时使用各项交付物的当前内容。</p></div></div><div class="package-list">${data.packages.map((pkg) => `<article class="package-item"><div style="display:flex;justify-content:space-between;gap:10px"><h3>${esc(pkg.name)}</h3>${status(pkg.version,"normal")}</div><p>${esc(pkg.scope)} · ${pkg.deliverableIds.length} 项成果</p><p>负责人 ${memberName(pkg.owner)} · ${pkg.formedAt}</p><div style="margin-top:9px;display:flex;gap:6px"><button class="btn compact" data-action="package-detail" data-id="${pkg.id}">查看来源</button><button class="btn compact" data-action="download-package" data-id="${pkg.id}">${icon("download")}模拟下载</button></div></article>`).join("")}</div></section>
  ...
  function renderReports() {
    ...
    return `${head("项目报告", "报告由当前项目事实生成，可补充判断后发布或导出。", `<button class="btn" data-action="export-report" data-type="png">${icon("download")}导出移动端长图</button><button class="btn primary" data-action="export-report" data-type="pdf">${icon("download")}导出 PDF</button>`)}
      ...
      <article id="report-sheet" class="report-sheet"><div class="report-title"><h2>${esc(data.project.name)}项目报告</h2><p>${labels[ui.reportPeriod]} · 生成时间 2026-08-21 21:30 · 基于当前项目事实</p></div>
        <section class="report-section"><h3>一、整体判断</h3><p>项目处于${esc(data.project.phase)}。三项目标持续推进，体验与运行主线已进入联合验证；当前需优先处理两项入池审批和一组跨 KR 硬依赖互锁。</p></section>
        ...
        <section class="report-section"><h3>三、卡点与待决策</h3><ul>${data.risks.slice(0,4).map((risk) => `<li>${risk.taskId}：${esc(risk.reason)}，当前待行动人 ${memberName(risk.actionOwner)}；${esc(risk.impact)}。</li>`).join("")}</ul></section>
  ...
  function taskApprovalItems(taskId) {
    return [
      ...data.entryApprovals.filter((item)=>item.taskId===taskId).map((record)=>({kind:"entry",label:"任务入池审核",record})),
  ...
  function taskRelationCard(rel, taskId) {
    ...
    if (!other) return "";
    const deliverable = data.deliverables.find((item)=>item.relationId===rel.id && item.state==="已生效");
    return `<button class="relation-card" data-action="task-detail" data-id="${other.id}"><span class="relation-card-main"><b>${other.id} · ${esc(other.title)}</b><small>${kr?.id || "—"} · ${relationTypeLabel(rel.type)} · ${esc(deliverable?.name || rel.label)}</small></span><span class="relation-card-meta"><span>${memberName(other.owner)} · ${other.status}</span></span>${icon("chevron")}</button>`;
  }
  function workFocusPanel(task, context) {
    ...
    if (context.focusType==="risk" || context.focusType==="risk-timeout") {
      ...
      const title=risk?`${risk.level} · ${risk.reason}`:`审批超时 · ${approval?.id || context.focusId}`, detail=risk?`${risk.impact} · 待行动人 ${memberName(risk.actionOwner)}`:`当前环节已等待 ${approval?.waitingDays || 0} 天，超过项目阈值 ${approvalTimeout()} 天`;
    ...
    return `<section id="task-focus-target" class="${focusClass}"><div><span class="focus-kicker">当前待处理任务</span><h3>${task.id} · ${esc(task.title)}</h3><p>${task.status} · 截止 ${task.due} · 任务负责人 ${memberName(task.owner)}</p></div></section>`;
  }
  function taskOverviewTab(task, context=ui.taskDrawerContext) {
    const kr=getKr(task.krId), stage=taskStage(task);
    ...
    const current=data.deliverables.filter((item)=>item.taskId===task.id && item.state==="已生效"), candidates=data.deliverables.filter((item)=>item.taskId===task.id && item.state==="审核中");
    ...
    const relationSection=(incoming.length||outgoing.length)?`<section class="drawer-section"><div class="section-title-row"><h3>协作关系</h3><button class="link-btn" data-action="focus-graph" data-id="${task.id}">在关系图谱中查看 →</button></div>${incoming.length?`<div class="relation-group"><b>直接上游 · ${incoming.length}</b>${incoming.map((rel)=>taskRelationCard(rel,task.id)).join("")}</div>`:""}${outgoing.length?`<div class="relation-group"><b>直接下游 · ${outgoing.length}</b>${outgoing.map((rel)=>taskRelationCard(rel,task.id)).join("")}</div>`:""}</section>`:"";
    return `<div class="task-tab-panel">${workFocusPanel(task,context)}<section class="drawer-section"><h3>基础信息</h3><div class="task-info-list"><div class="task-info-row"><span>状态</span><div>${status(task.status)}</div></div><div class="task-info-row"><span>负责人</span><strong>${name(task.owner)}</strong></div><div class="task-info-row"><span>参与人</span><strong>${task.participants.length ? task.participants.map(memberName).join("、") : "未设置"}</strong></div><div class="task-info-row"><span>开始／截止</span><strong>${task.start} — ${task.due}</strong></div><div class="task-info-row"><span>进度</span><strong>${task.progress == null ? "未填写" : `${task.progress}%`}</strong></div><div class="task-info-row"><span>当前环节／待行动人</span><strong>${stage.stage} · ${memberName(stage.actor)}</strong></div><div class="task-info-row task-info-long"><span>任务说明</span><strong>${esc(task.description || "未填写")}</strong></div><div class="task-info-row task-info-long"><span>量化标准</span><strong>${esc(task.acceptanceCriteria || `形成可供 ${memberName(task.receiver)} 接收的“${task.outputName}”，并通过 KR 终审。`)}</strong></div></div></section>
      ...
      <section class="drawer-section"><h3>当前交付物 <span class="meta">${current.length} 项</span></h3>${candidates.length?`<div class="notice warning" style="margin-bottom:10px">有 ${candidates.length} 项更新审核中，候选内容请在“审核”Tab 查看；当前内容继续有效。</div>`:""}${deliverableCards || `<div class="empty compact-empty">尚无已生效的当前交付物</div>`}</section>${relationSection}</div>`;
  }
  ...
  function openTaskDrawer(taskId, tab="overview", context=ui.taskDrawerContext) {
    ...
    const tabs=`<nav class="task-tabs" aria-label="任务详情"><button class="${tab==="overview"?"active":""}" data-action="task-tab" data-id="${task.id}" data-value="overview">任务概况</button><button class="${tab==="audit"?"active":""} ${needsAction?"needs-action":""}" data-action="task-tab" data-id="${task.id}" data-value="audit">审核 ${pending}</button><button class="${tab==="discussion"?"active":""}" data-action="task-tab" data-id="${task.id}" data-value="discussion">讨论 ${comments}</button></nav>`;
    ...
    if (["tasks","mywork"].includes(ui.taskDrawerSource) && !(context?.source==="mywork" && (["waiting","risks"].includes(context.sourceGroup) || ["receipt","input"].includes(context.focusType)))) {
      if (ui.taskDrawerSource==="tasks") footer=`<button class="btn" data-action="report-risk" data-id="${task.id}">${icon("alert")}上报卡点</button>`;
    ...
    $("#drawer-root .drawer")?.classList.add("task-drawer");
  ...
  function createTaskModal(inviteId="") {
    ...
    const body = `<form id="create-task-form" data-invite-id="${invite?.id || ""}">${memberDatalist("task-member-options", false)}${invite?`<div class="notice" style="margin-bottom:12px"><b>${invite.krId} 任务创建邀请</b><br>${esc(invite.note)}<br><span class="meta">只有通过本邀请提交关联任务，邀请才会退出。</span></div>`:""}<div class="task-sheet"><div class="task-sheet-head"><span>所属 KR</span><span>任务名称</span><span>负责人</span><span>开始 / 截止</span><span>预期交付物</span><span></span></div><div id="task-sheet-rows">${taskDraftRow(invite)}</div></div><div class="task-sheet-actions"><button type="button" class="btn compact" data-action="add-task-row">${icon("plus")}继续添加任务</button><span class="meta">可连续录入多项任务，保存后统一提交各自所属 KR 的入池审批。</span></div><div class="notice" style="margin-top:12px">任务提交后处于“待入池审批”；所属 KR 负责人通过后，才进入执行池并变为“未开始”。</div></form>`;
    openModal(invite?"响应任务创建邀请":"创建任务", invite?`${invite.krId} · ${memberName(invite.inviter)} 发起`:"按 KR 连续录入任务骨架并指定负责人", body, `<button class="btn" data-action="close-modal">取消</button><button class="btn primary" data-action="submit-new-task">提交入池审批</button>`);
  ...
  function taskDraftRow(invite=null) {
    ...
    return `<div class="task-sheet-row"><div class="task-sheet-cell"><select name="krId" class="field" aria-label="所属 KR">${krOptions}</select></div><div class="task-sheet-cell"><input name="title" class="field" required value="${esc(invite?"验证现场联动异常回退":"新增协作任务")}" aria-label="任务名称"></div><div class="task-sheet-cell">${memberSearchInput("owner", defaultOwner, "task-member-options", "任务负责人")}</div><div class="task-sheet-cell sheet-date-pair"><input type="date" name="start" class="field" value="2026-09-12" aria-label="开始时间"><input type="date" name="due" class="field" value="2026-09-21" aria-label="截止时间"></div><div class="task-sheet-cell"><input name="outputName" class="field" value="任务成果记录" aria-label="预期交付物"></div><button type="button" class="icon-btn" data-action="remove-task-row" aria-label="删除该任务行">${icon("close")}</button></div>`;
  }
  ...
  function configureInputModal(taskId) {
    const task = getTask(taskId); if (!task) return;
    const body = `<form id="input-source-form" data-task-id="${taskId}"><div class="radio-cards"><label class="radio-card active" data-action="input-mode" data-value="task"><input type="radio" name="mode" value="task" checked hidden><b>从已有任务选择</b><span>优先复用系统内已生效交付物</span></label><label class="radio-card" data-action="input-mode" data-value="person"><input type="radio" name="mode" value="person" hidden><b>指定项目成员提供</b><span>没有合适来源任务时使用</span></label></div><div id="input-task-panel"><label class="search-box" style="margin-top:12px;width:100%">${icon("search")}<input id="source-search" placeholder="按任务、O、KR、负责人或交付物搜索" /></label><div class="input-source-list" id="source-results">${data.tasks.filter((x) => x.id !== taskId).slice(0,6).map((x) => `<label class="source-option"><input type="radio" name="sourceTask" value="${x.id}"><span><b>${x.id} · ${esc(x.title)}</b><span class="meta">${x.krId} · ${memberName(x.owner)} · ${esc(x.outputName)}</span></span></label>`).join("")}</div></div><div id="input-person-panel" class="hidden form-grid" style="margin-top:12px"><div class="form-row"><label>对接人</label><select name="provider" class="field">${data.members.filter((m) => !["readonly","lead"].includes(m.permission)).map((m) => `<option value="${m.id}">${m.displayName} · ${m.role}</option>`).join("")}</select></div><div class="form-row"><label>期望时间</label><input class="field" name="due" value="09-22"></div><div class="form-row full"><label>需要提供的内容</label><input class="field" name="inputName" value="现场联调确认记录"></div><div class="form-row full"><div class="notice">审批通过后发送站内通知。对接人只能“同意接收”；不会自动创建新任务。</div></div></div></form>`;
  ...
  function handleInput(event) {
    ...
    if (event.target.id === "source-search") filterSourceResults(event.target.value);
  ...
  function handleChange(event) {
    ...
    if (event.target.name === "sourceTask") $$(".source-option",$("#modal-root")).forEach((x)=>x.classList.toggle("selected",x.contains(event.target)));
  ...
  function filterSourceResults(query) {
    const q=query.toLowerCase();
    const matches=data.tasks.filter((x)=>`${x.id}${x.title}${x.krId}${x.owner}${memberName(x.owner)}${x.outputName}`.toLowerCase().includes(q)).slice(0,8);
    $("#source-results").innerHTML=matches.map((x)=>`<label class="source-option"><input type="radio" name="sourceTask" value="${x.id}"><span><b>${x.id} · ${esc(x.title)}</b><span class="meta">${x.krId} · ${memberName(x.owner)} · ${esc(x.outputName)}</span></span></label>`).join("") || `<div class="empty">没有匹配来源，可切换为指定成员提供。</div>`;
  }
  function saveInputSource() {
    const form=$("#input-source-form"), fd=new FormData(form), taskId=form.dataset.taskId, mode=fd.get("mode");
    if(mode==="task") {
      const source=fd.get("sourceTask"); if(!source)return toast("请选择一个已有任务。","error");
      const sourceTask=getTask(source), target=getTask(taskId), deliverable=data.deliverables.find((d)=>d.taskId===source);
      const id=`R${String(data.relations.length+1).padStart(2,"0")}`;
      data.relations.push({id,from:source,to:taskId,type:"input",necessity:"必要",label:sourceTask.outputName,pathId:null,state:deliverable?.state==="已生效"?"已就绪":"等待当前交付物生效"});
      saveData(`为 ${taskId} 绑定系统内输入来源 ${source}`);closeModal();toast(`已建立 ${source} → ${taskId} 的交付物边；${deliverable?.state==="已生效"?"输入已就绪":"等待来源当前交付物生效"}。`,"success");
    } else {
      const id=`IR${String(data.inputRequests.length+1).padStart(2,"0")}`;
      data.inputRequests.push({id,taskId,inputName:fd.get("inputName"),provider:fd.get("provider"),requester:ui.identity,state:"待配置审批",due:fd.get("due"),necessity:"必要",createdAt:"刚刚",waitingDays:0,impact:`影响 ${taskId} 及其下游任务`});
      data.changeRequests.push({id:`CR${String(data.changeRequests.length+1).padStart(2,"0")}`,taskId,submitter:ui.identity,krOwner:getKr(getTask(taskId).krId).owner,state:"待审批",field:"输入来源",oldValue:"未配置",newValue:`${memberName(fd.get("provider"))} 提供 ${fd.get("inputName")}`,reason:"现有任务中没有合适输入源",inputRequestId:id});
      saveData(`提交 ${taskId} 人工输入来源配置审批`);closeModal();toast(`输入来源已提交 KR 负责人审批；通过前不会通知 ${memberName(fd.get("provider"))}。`,"success");
    }
  }
  function submitNewTask() {
    ...
    saveData(`创建 ${created.join("、")} 并提交入池审批`); closeModal(); toast(`${created.length} 项任务已提交入池审批，尚未进入执行池。`, "success"); if (["tasks","mywork"].includes(route())) renderPage();
  }
  function confirmCompletion() {
    ...
    const candidateIds=rows.map((row)=>{const value=(name)=>$(`[name="${name}"]`,row)?.value.trim()||"",file=$("input[data-upload-control]",row)?.files?.[0],id=`D${String(nextNumber++).padStart(2,"0")}`;data.deliverables.push({id,krId:task.krId,taskId,relationId:data.relations.find((rel)=>rel.from===taskId)?.id||"待建立",name:value("name"),owner:task.owner,submittedBy:ui.identity,receiver:task.receiver,state:"审核中",formedAt:"刚刚",file:file?.name||"",fileType:file?uploadFileType(file):"文件",fileSize:file?formatUploadSize(file.size):"—",submittedAt:"刚刚",effectiveAt:""});return id;});
  ...
  function riskActionsModal(id){const risk=data.risks.find((x)=>x.id===id);openModal("卡点协调",`${risk.taskId} · ${risk.reason}`,`<div class="radio-cards"><div class="radio-card active"><b>指定协调人</b><span>由总推进人协调上下游</span></div><div class="radio-card"><b>升级待决策</b><span>进入项目总负责人待决策</span></div></div><div class="notice warning" style="margin-top:12px">如需改期，请另行提交关键字段变更审批；本操作不自动修改日期。</div>`,`<button class="btn" data-action="close-modal">取消</button><button class="btn primary" data-action="confirm-risk-action" data-id="${id}">确认协调</button>`);}
